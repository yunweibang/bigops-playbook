...
- name: example
  ...
  tasks:
    ...
    - name: 安装windows_exporter服务
      win_shell: cmd.exe /c sc create windows_exporter binPath='\"{{ dest_path }}/windows_exporter.exe\" --telemetry.addr :9100 --collectors.enabled=\"cpu,memory,cs,logical_disk,net,os,process,tcp,system,textfile\" --collector.textfile.directory \"{{ dest_path }}/key/\"' start=auto
      ignore_errors: yes
